...
def a_star(start_x,end, row, grid,grid_width,grid_height,grid_height2,limit):
    open_lst = []
    close_lst = []
    ...
    while(1):
       ...
       # print(len(open_lst))
        ...
        for direction in directions:
            ...
            if ((neighbr_pt.x > grid_height2) or (neighbr_pt.x < grid_height) or (neighbr_pt.y > grid_width) or (neighbr_pt.y < 0)):
                
                #print("peak 1: " + str(grid_height))
                #print("peak 2: " + str(grid_height2))
                #print(neighbr_pt.x)
                continue
            ...
            g_scr = g_score_cal(direction)+ current.g_score +punish 
            h_scr = heuristics(neighbr_pt,end_pt)
            f_scr = h_scr+g_scr #g_scr + h_scr + current.g_score
...
image = np.asarray(Image.open('/Users/nikee/Desktop/handwritten_recognition/image-data/P123-Fg001-R-C01-R01-binarized.jpg')) #P342-Fg001-R-C01-R01-binarized
...
#save_img  = ImageOps.mirror(save_img)
...
#average distance between valleys - for search constraints
...
for i in range(0,len(valleys)):
    i = 4
    end_nd = a_star(0, img_width, valleys[i], save_img,img_width-1,valleys[i]-avg*2,valleys[i]+int(avg/2),avg*2)
    lower_line_max = 0
    upper_line_min = 99999
    line = []
    while(end_nd!=None):
        save_img[end_nd.cordinates.x][end_nd.cordinates.y] = 160
       # save_img[valleys[i]+int(avg/2)][end_nd.cordinates.y] = 60
        #save_img[valleys[i]-avg*2][end_nd.cordinates.y] = 30
        #save_img[mountains[i]][end_nd.cordinates.y] = 100
